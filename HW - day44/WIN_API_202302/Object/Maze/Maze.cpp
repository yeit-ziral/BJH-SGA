...
void Maze::CreateMaze()
{
	...
	for (int y = 0; y < _poolCountY; ++y)
	{
		for (int x = 0; x < _poolCountX; ++x)
		{
			...
			// 랜덤으로 우측 혹은 아래로만 뚫었을 때 길이 안생기는것 예외처리
			if (y == _poolCountY - 2)
			{
...
void Maze::CreateMazeByKruskal()
{
	...
	// 모든 정점에 대하여 이어져있는 간선들을 랜덤벨류로 설정하고 edges에 추가
	for (int y = 0; y < _poolCountY; y++)
	{
		for (int x = 0; x < _poolCountX; x++)
		{
	...
	DisjoinSet sets(_poolCountX * _poolCountY);
	...
	for (auto edge : edges)
	{
		...
		_blocks[edgeX][edgeY]->SetType(MazeBlock::BlockType::ABLE);
	}
}
